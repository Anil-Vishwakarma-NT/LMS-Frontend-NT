...
const UserVideoModal = ({ isOpen, fileName, resourceType, onClose, contentId, courseId }) => {
  const lastPositionRef = useRef(0);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Generate video URL based on resourceType or file extension
  ...
  const handleClose = () => {
    ...
    onClose();
    setVideoUrl(null); // clear URL on close
  };

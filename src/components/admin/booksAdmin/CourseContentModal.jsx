...
const CourseContentModal = ({
  ...
  // Fetch course name whenever courseId changes or modal opens
  useEffect(() => {
    const loadCourseName = async () => {
      try {
        if (courseId) {
  ...
  // Set form values when modal opens or selectedContent changes
  useEffect(() => {
    if (isModalOpen) {
      if (selectedContent) {
        form.setFieldsValue({
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      if (error.errorFields) {
        // Validation errors are handled by AntD form UI automatically
      } else {
        console.error("Error saving course content:", error);
        setToastMessage("Failed to save course content.");
        setToastType("error");
